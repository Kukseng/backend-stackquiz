...
public class QuizMapperImpl implements QuizMapper {
    ...
    @Override
    public Quiz duplicateQuiz(Quiz originalQuiz) {
        ...
        quiz.setParentQuiz( duplicateQuiz( originalQuiz ) );
        quiz.setTitle( originalQuiz.getTitle() );
        quiz.setDescription( originalQuiz.getDescription() );
        quiz.setThumbnailUrl( originalQuiz.getThumbnailUrl() );
        quiz.setVisibility( originalQuiz.getVisibility() );
        quiz.setQuestionTimeLimit( originalQuiz.getQuestionTimeLimit() );
        quiz.setDifficulty( originalQuiz.getDifficulty() );
        quiz.setStatus( originalQuiz.getStatus() );
        quiz.setVersionNumber( originalQuiz.getVersionNumber() );
        List<QuizCategory> list = originalQuiz.getQuizCategories();
        if ( list != null ) {
            quiz.setQuizCategories( new ArrayList<QuizCategory>( list ) );
        }
        List<Question> list1 = originalQuiz.getQuestions();
        if ( list1 != null ) {
            quiz.setQuestions( new ArrayList<Question>( list1 ) );
        }
